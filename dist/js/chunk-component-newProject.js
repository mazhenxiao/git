...
var _componentNewProjectCount = __webpack_require__(963);

var _componentNewProjectCount2 = _interopRequireDefault(_componentNewProjectCount);

var _toolsDynamicTable = __webpack_require__(810);
...
var NewProject = function (_React$Component) {
    ...
    _createClass(NewProject, [{
        ...
        value: function componentWillMount() {
            var th = this;
            var local = th.props.location;
            var status = local.query["status"];
            var dataKey = local.query["dataKey"]; //local.query["dataKey"];
            var projectId = "";
        ...
        key: "EVENT_CLICK_PERIPHERAL",
        value: function EVENT_CLICK_PERIPHERAL() {
            //点击外设条件
        ...
        key: "BIND_CHECKINewLand",
        value: function BIND_CHECKINewLand(da) {
            //地块初次获取到以后检查是否合法，解决
            var _da = da.map(function (el, ind) {
                el.FieldList.forEach(function (ee, ii) {
                    var reg = eval("(" + ee.regExp + ")"),
                        val = "";
                    if (reg && reg.type.indexOf("number") >= 0) {
                        val = new Function("return " + ee.val)() || 0;
                        if (reg.max && val > eval(reg.max)) {
                            //如果当前值大于传入限制最大值
                            ee.test.check = false;
                        }
        ...
        value: function BIIND_FIST_LAND() {
            ...
            var th = this;
            var id = th.state.projectId; //"A91BB3051A0848319B45D3D527AC4103" //this.props.location.state.id;
            var INewLand = new Promise(function (resolve, reject) {
                iss.ajax({
            ...
            var IProjectLandsInfo = new Promise(function (resolve, reject) {
                iss.ajax({ //获取已有地块
                    ...
                    success: function success(d) {
                        ...
                        if (d.rows) {
                            ...
                            var da = {};
                            th.BIND_CHECKINewLand(d.rows).forEach(function (el, ind) {
                                if (ind == 0) {
                                    //初次加载地块
            ...
            var ILandsStatistics = new Promise(function (resolve, reject) {
                iss.ajax({
            ...
            Promise.all([INewLand, IProjectLandsInfo, ILandsStatistics]).then(function (arg) {
                //console.log("all");
                    ...
                    if (el.id == i) {
                        var _ret = function () {
                            ...
                            if (arr) {
                                ...
                                if (fixed) {
                                    _exec = parseFloat(_exec).toFixed(fixed);
                                }
        ...
        value: function BIND_LAND_BTN() {
            ...
            for (var i in li) {
                ...
                map.push(_react2.default.createElement(
                    "li",
                    { className: li[i].LandId == this.state.pid ? "active" : "", onClick: this.EVENT_CLICK_LANDBTN.bind(this, li[i].LandId), key: g, "data-id": li[i].LandId },
                    name,
                ...
                if (el.exec && !reg.exec(el.exec)) {
                    if (arr && arr[0]) {
                        var _ret2 = function () {
                            ...
                            var n_n = eval(_str.join("+")) || 0;
                            if (fixed) {
                                n_n = parseFloat(n_n).toFixed(fixed);
                            }
                        ...
                        var _n_n_ = eval(str) || 0;
                        if (fixed) {
                            _n_n_ = parseFloat(_n_n_).toFixed(fixed);
                        }
        ...
        value: function EVENT_CLICK_SAVE(callback) {
            ...
            if ($.trim(th.state.NewProjectCountDATA.PROJECTNAME) && th.state.NewProjectCountDATA.CITY) {
                if (status == "upgrade") {} else {
                    if (!th.state.NewProjectCountDATA.checkName) {
                        iss.popover({ content: "项目名称已存在请重新填写！"
                        });return;
                    }
                }
        ...
        value: function GET_CHECKED() {
            ...
            var status = th.state.status;
            if (status == "edit") {
                //如果是编辑状态下项目名称没有变更，改变checkName为true
            ...
            if (!th.BIND_SELF_NewProjectCount()) {
                //验证基础数据
                iss.popover({
            ...
            var _check = th.BIND_VALID_DATA(); //检查地块数据
            if (!_check[0]) {
                //验证地块
        ...
        value: function BIND_SELF_NewProjectCount() {//重定向到NewProjectCount模块内
                ...
                _react2.default.createElement(
                    ...
                    _react2.default.createElement(
                        ...
                        _react2.default.createElement(
                            ...
                            _react2.default.createElement(
                                "a",
                                { className: "saveIcon ", onClick: this.EVENT_CLICK_SAVE.bind(this, "zc"), href: "javascript:void(0);" },
                                "\u6682\u5B58"
                            ),
                            _react2.default.createElement(
                                "a",
                    ...
                    _react2.default.createElement(_componentNewProjectCount2.default, { ref: "NewProjectCount", local: th.props.location, NewProjectCountDATA: th.BIND_NewProjectCountDATA.bind(th), save: th.EVENT_CLICK_SAVE.bind(this), status: th.state.status, projectId: th.state.projectId, point: th.BIND_NewProjectCount.bind(this) })
                ),
                _react2.default.createElement(
                    "section",
                    null,
                    ...
                    _react2.default.createElement(
                        "article",
                        { className: this.state.propsDATA.length ? "tools-dynamicTable hide" : "tools-dynamicTable hide" },
                        _react2.default.createElement(
                            "ul",
                            { className: "row" },
                            _react2.default.createElement(
                                ...
                                _react2.default.createElement("label", { className: "require" }),
                                _react2.default.createElement(
                                    "i",
                                    null,
                                ...
                                _react2.default.createElement(
                                    ...
                                    _react2.default.createElement(
                                        "button",
                                        { className: "btn btn-default", onClick: this.EVENT_CLICK_PERIPHERAL.bind(this) },
                                        "\u5916\u90E8\u8BBE\u8BA1\u6761\u4EF6"
                                    )
                                )
                            )
...
var _toolsCity = __webpack_require__(952);
